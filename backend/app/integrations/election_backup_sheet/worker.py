...
class ElectionBackupSheetWorker:
    ...
    def process_pending_deletions(self) -> None:
        ...
        for row in pending:
            ...
            try:
                ...
                self.mark_deletion_complete(row["id"], spreadsheet_id)
    ...
    @staticmethod
    def mark_deletion_complete(deletion_id: str, spreadsheet_id: str) -> None:
        with psycopg.connect(database_url(), row_factory=dict_row) as conn:
            with conn.cursor() as cur:
                cur.execute(
                    """
    ...
    @staticmethod
    def mark_deletion_error(deletion_id: str, message: str) -> None:
        with psycopg.connect(database_url(), row_factory=dict_row) as conn:
            with conn.cursor() as cur:
                cur.execute(
                    """
